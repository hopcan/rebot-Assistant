...
class reBotArm_handle:
    ...
    def __init__(self, interface,arm_version = "rebotDM",config_path = None, log_handle = None):  #rebotDM or rebotRS
        ...
        for motor_can_id in list(range(1, 8)):
            ...
            if(self.arm_version=="rebotDM"):
                ...
                try:
                    ...
                    self.log_handle.emit(f"[error] motor_can_id=0x{motor_can_id:02X}: {e}")
                    return False
            elif(self.arm_version=="rebotRS"):
    ...
    def check_ids_valid(self):
        ...
        for motor_can_id in list(range(1, 8)):
                if self.arm_version=="rebotDM":
                    motor_master_id = 0x10 + motor_can_id
                    self.log_handle.emit(f"Checking ID 0x{motor_can_id:02X} ({len(found_motors)+1}/{len(self.expected_ids)})")
                    try:
                        ...
                        if can_id == motor_can_id and master_id == motor_master_id :
                            ...
                            self.log_handle.emit(f"can id: {can_id:02x} mismatch, offset is wrong")
                            self.motors[motor_can_id].handle.close()
                            return False  
                    except Exception:
                        # read error, no this can id
                        self.log_handle.emit(f"[no respond] motor_can_id=0x{motor_can_id:02X}")
                        return False
                    ...
                    try:
                        ...
                        # scan error
                        self.log_handle.emit(f"[no respond] no this motor_can_id=0x{motor_can_id:02X}")
         
        if len(found_motors) == len(self.expected_ids) :
            self.log_handle.emit("All ids are valid !")
            return True
        ...
        if self._add_motor_to_ctrl():
            if self.check_ids_valid() :
                try :
                    ...
                    self.log_handle.emit("enable all motors failed")
                    return False
            else:
                self.log_handle.emit("Not all IDs are valid. Cannot connect.")
                return False
        else :
            self.log_handle.emit("Failed to add motors to controller. Cannot connect.")
            return False
    ...
    def disconnect(self):
        ...
        try:
            ...
            self.log_handle.emit(f"shutdown 失败: {e}")
        
        try:
            self.ctrl.close()
        except Exception as e:
            self.log_handle.emit(f"close 失败: {e}")
        self.is_connected = False
    ...
    def set_zero_position(self):
        ...
        for motor_can_id in list(range(1, 8)):
            try:
                ...
                self.log_handle.emit(f"motor {motor_can_id}: set zero failed")
                return False
            self.motors[motor_can_id].handle.request_feedback()
            state = self.motors[motor_can_id].handle.get_state()
            if state.pos < 0.1 and state.pos > -0.1:
                self.log_handle.emit(f"motor {motor_can_id}: set zero successfully")
            else :
                self.log_handle.emit(f"motor {motor_can_id}: set zero failed")
        self.ctrl.enable_all()
        ...
        for motor_can_id in list(range(1, 8)):
            ...
            if self.motors[motor_can_id].use_mode == "MIT":
                try:
                    ...
                    # self.log_handle.emit(f"motor {motor_can_id}: change mode to MIT successfully")
                except Exception:
                    self.log_handle.emit(f"motor {motor_can_id}: change mode to MIT failed")
                    return False
            elif self.motors[motor_can_id].use_mode == "POS_VEL":
                try:
                    self.motors[motor_can_id].handle.ensure_mode(Mode.POS_VEL, timeout_ms=1000)
                    # self.log_handle.emit(f"motor {motor_can_id}: change mode to POS_VEL successfully")
                except Exception:
                    self.log_handle.emit(f"motor {motor_can_id}: change mode to POS_VEL failed")
                    return False
    ...
    def move_to_joint_positions(self,positions,velocity=None,torque=None):
        ...
        if len(positions) != len(self.expected_ids):
            self.log_handle.emit("Positions list length does not match number of motors.")
            return False
        for motor_can_id in list(range(1, 8)):
